...
- locale.jplug    零代码语言包插件：安装即自动启用，验证语言贡献 → LangPicker → 搜索占位符切法语 → 卸载回落。
"""
...
LOCALE_MANIFEST = """id = "e2e-locale"
name = "E2E Français"
version = "1.0.0"
apiVersion = "0.4"
description = "e2e 夹具：零代码语言包"
...
# 只翻一个可断言的 key：顶栏搜索占位符（其余自动回落 base=en）
LOCALE_JSON = '{"topbar.search": "Rechercher des notes\\u2026"}\n'
...
def main() -> None:
	...
	write_zip('locale.jplug', {
		'manifest.toml': LOCALE_MANIFEST.encode(),
		'assets/locales/fr.json': LOCALE_JSON.encode(),
	})
