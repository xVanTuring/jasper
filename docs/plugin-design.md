...
POST   /api/plugins/{id}/commands/{cmd}   （阶段 3）
POST   /api/plugins/{id}/ui/{view}        （阶段 3）
POST   /api/ai/complete                   （阶段 3）
```
...
3. **阶段 3 — 受控能力 + widget 宿主**：`host:ai`/`notes:*` + 内置 `chat`/`form` widget + server-driven UI + commands/ui 端点 → **AI 插件端到端**。UI 去静态化。
