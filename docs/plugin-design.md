...
- `host:fetch`（插件网络出口）：**暂不做**（AI 已由 host 代理覆盖）。
...
2. **后端 Rust-only** → host 与插件**共享 `joplin-core` 类型 + JSON serde ABI**（§6.2），无需 IDL。
...
└───────────────────────────────┬─────────────────────────────────┘
                                 │ 共享 joplin-core 类型 + JSON ABI
┌───────────────────────────────┴───────── 插件 (.wasm, Rust) ────┐
│  依赖 joplin-core，拿同一套 Note/Folder；默认零能力              │
...
- host 与插件都依赖 `joplin-core`，拿**完全相同**的 `Note`/`Folder`。
...
## 9. 新增 API（草拟）
```
GET    /api/plugins                       已装列表 + manifest(JSON) + 启停状态
POST   /api/plugins/install               本地：上传 zip(body) 安装
DELETE /api/plugins/{id}                  卸载
POST   /api/plugins/{id}/enable           启用/停用
GET/PUT /api/plugins/{id}/settings        读/写插件设置
POST   /api/plugins/{id}/commands/{cmd}   调用命令（→ 后端 wasm 导出）
POST   /api/plugins/{id}/ui/{view}        取动态 widget 声明树（server-driven UI）
POST   /api/ai/complete                   host 代理 AI（密钥在 host）
```
钩子（无新端点）：`on-before-save` 注入 `update_note`/`create_note`。

## 10. 代码落点
- 新 crate `plugin-sdk/`（ABI 宏 + 重导出 `joplin-core` 类型），path 依赖，沿用"无 workspace"。
...
## 11. 分阶段路线
1. **阶段 1 — 主题**：CSS 令牌化重构（补全令牌 + `data-theme` + 文档化）+ 声明式主题加载。零运行时，先做。
2. **阶段 2 — 后端骨架**：`plugin-sdk` + wasmi 加载器 + zip 安装(本地) + `on-before-save` 钩子 + 示例格式化插件。跑通 = 后端路线成立。
3. **阶段 3 — 受控能力 + widget 宿主**：`host:ai`/`notes:*`/`settings` + 内置 `chat`/`form` widget + server-driven UI → **AI 插件端到端**。UI 去静态化。
...
- [ ] server-driven UI 声明树的 **schema 细节**——阶段 3 前定；建议先用 `{type, props, children}` 一层树。
```
